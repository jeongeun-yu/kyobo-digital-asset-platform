...
[교보생명 내부망]       Java — 확정
  Core Banking, 내부 DB, 감사 로그
        ↕ REST API
[DMZ]                  Node.js / TypeScript 마이크로서비스 — 확정
...
|---|---|---|
| 내부망 런타임 | ✅ **확정** | Java |
| DMZ 런타임 | ✅ **확정** | Node.js / TypeScript |
| 데이터베이스 | 🔲 **유력** | Oracle (사내 고성능 Oracle 사용 중 — 유선 언급) |
| 프레임워크 | 🔲 미결정 | — |
| 메시지 큐 | 🔲 미결정 | — |
| 보안점검 Node.js 허용 여부 | ⚠️ **확인 필요** | Java 외 허용 여부 서면 확인 필요 |

> **보안점검 관련**: Node.js가 사내 보안점검 허용 목록에 있는지 공식 확인 필요. DMZ는 내부망이 아닌 외부 접점 영역이므로 내부망 기준과 다를 수 있음. 서면 확인 전까지는 이 리스크가 열려있다.
...
|---|---|---|---|
| 1 | 내부망 런타임 | ✅ **확정** | Java |
| 2 | DMZ 런타임 | ✅ **확정** | Node.js / TypeScript |
