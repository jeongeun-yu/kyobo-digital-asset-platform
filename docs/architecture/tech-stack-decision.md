...
> **WAS 경유 제약 관련 추가 확인 필요**: 내부망→DMZ 방향의 API 호출이 WAS를 통해서만 가능한 구조라면, DMZ 서비스가 내부 Java 시스템으로 콜백(callback)하는 방식에 제약이 생길 수 있다. 구체적인 네트워크 정책은 교보DTS 인프라팀 확인 필요.
...
- [ ] Java Spring으로 전환 (web3j 기반, 위 이슈 인지 후)
