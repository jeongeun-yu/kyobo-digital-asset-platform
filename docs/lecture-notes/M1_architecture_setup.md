...
Phase 1에서 사용할 블록체인은 교보생명과 VASP 파트너사 간 협의를 통해 결정된다. 이 과정에서 구현하는 스켈레톤 코드는 EVM 호환 체인이면 어디든 배포 가능하도록 설계되어 있다 — 컨트랙트 코드, 배포 스크립트, VASP 연동 로직 모두 체인 주소만 바꾸면 동작한다. `hardhat.config.ts`에 현재 `sepolia`가 설정된 것은 **교육 실습을 위한 플레이스홀더**다. 프로덕션 체인 결정과 무관하다.
...
교육 실습에서 Ethereum Sepolia 테스트넷을 사용하는 이유는 단순하다: 테스트 토큰이 무료이고, 배포 속도가 빠르며, 이더리움 생태계 도구(Hardhat, ethers.js, Etherscan)가 동일하게 동작한다. 테스트넷 선택은 프로덕션 체인 결정과 완전히 별개다.
