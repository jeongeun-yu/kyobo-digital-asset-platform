...
[KyoboNFT 스마트컨트랙트]       ← M2·M3 구현 영역
  │ (블록체인 — 미결정, 교육용 실습은 Polygon Amoy 테스트넷)
  ▼
...
**어떤 블록체인을 쓸 것인가 — 아직 결정되지 않은 사안**

Phase 1에서 사용할 블록체인은 교보생명과 VASP 파트너사 간 협의를 통해 결정된다. 이 과정에서 구현하는 스켈레톤 코드는 EVM 호환 체인이면 어디든 배포 가능하도록 설계되어 있다 — 컨트랙트 코드, 배포 스크립트, VASP 연동 로직 모두 체인 주소만 바꾸면 동작한다. `hardhat.config.ts`에 현재 `polygon_amoy`가 설정된 것은 **교육 실습을 위한 플레이스홀더**다. 프로덕션 체인 결정과 무관하다.

체인 선택은 기술적 결정이기 이전에 비즈니스·거버넌스·규제 결정이다. 아래에서 실제 선택지와 트레이드오프를 정리한다.

**왜 이더리움 메인넷은 아닌가**

이것만큼은 거의 확실하다. 이더리움 메인넷은 비용이 너무 크다.

NFT 단건 발행(`mint`) 비용: 약 50,000 gas. 가스 가격 20 gwei(평시 기준) 기준 약 $1~3. 교보생명 월 수십만 건 발행 시 가스비만 수십억 원이다. 어떤 금융 서비스도 이 구조로는 운영할 수 없다. 이더리움 메인넷은 고가치 자산의 최종 정산 레이어로는 적합하지만, 대량 발행이 일어나는 애플리케이션 레이어로는 맞지 않는다.

**실제 선택지 — EVM 호환 레이어들**

이더리움 생태계는 "레이어 2(L2)" 확장 솔루션들을 발전시켜왔다. L2는 이더리움 메인넷(L1)의 보안을 그대로 상속하면서 거래 처리를 오프체인에서 수행하고 결과만 L1에 기록하는 구조다. 이더리움 L2와 사이드체인은 다르다.
...
Polygon PoS는 사이드체인이다. 자체 검증자 집합이 합의를 담당하고, 주기적으로 체크포인트 해시만 이더리움에 기록한다. 이더리움 L1이 Polygon의 개별 트랜잭션 유효성을 검증하지 않는다. Polygon 검증자 집합이 공모하면 이론적으로 자산을 위협할 수 있다. 이것이 금융기관이 "왜 사이드체인인가"라고 물을 때 답하기 어려운 이유다.

**교육 과정에서의 입장**

이 과정에서 배우는 스마트컨트랙트, VASP 연동, 원장 구조는 체인과 무관하다. 이 모든 것이 `IBlockchainAdapter` 뒤에 있기 때문이다. 체인이 어디로 결정되든 M2~M8에서 구현한 코드는 그대로 쓸 수 있다.

교육 실습에서 Polygon Amoy 테스트넷을 사용하는 이유는 단순하다: 테스트 토큰이 무료이고, 배포 속도가 빠르며, 이더리움 생태계 도구(Hardhat, ethers.js, Etherscan)가 동일하게 동작한다. 테스트넷 선택은 프로덕션 체인 결정과 완전히 별개다.
...
// 즉: AMOY_RPC_URL이 없으면 빈 문자열 → 네트워크 설정 비활성
```
