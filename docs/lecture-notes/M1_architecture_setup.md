...
**왜 이더리움이 아니라 Polygon인가**

"블록체인 = 이더리움"은 흔한 오해다. 이더리움은 생태계의 기준점이지만, 프로덕션에서 이더리움 메인넷을 그대로 쓰는 것은 비용 문제로 대부분 불가능하다.

이더리움 메인넷 NFT 단건 발행 비용: 약 50,000 gas. 가스 가격 20 gwei(평시) 기준 약 $1~3. 교보생명 월 50만 건 발행 시 가스비만 $50만~150만(약 7억~20억 원)이다. 서비스 자체가 성립하지 않는다.

Polygon PoS는 이더리움의 **사이드체인**이다. EVM과 100% 호환 — 같은 Solidity 코드, 같은 ABI, 같은 ethers.js, 같은 Hardhat이 그대로 동작한다. 차이는 합의 메커니즘이다. Polygon은 자체 검증자 집합(~100명)이 PoS로 블록을 생성하고, 주기적으로 체크포인트를 이더리움 메인넷에 기록해 보안을 담보한다.

| | 이더리움 메인넷 | Polygon PoS |
|---|---|---|
| NFT 발행 비용 | $1~3/건 | $0.001 미만/건 |
| 블록 생성 | ~12초 | ~2초 |
| Finality | ~12분 (Casper FFG) | ~수초 (자체) + 이더리움 체크포인트 |
| 보안 | 자체 PoS | 이더리움 체크포인트 담보 |
| EVM 호환 | 기준 | 100% 호환 |

국내 금융권 선행 사례도 Polygon이다. 신한은행 NFT 증명서, 하나은행 토큰 실험 모두 Polygon 위에서 진행됐다. VASP 파트너사들의 연동 레퍼런스도 이미 축적되어 있다.

`hardhat.config.ts`에 `polygon_amoy`(테스트넷)와 `polygon`(메인넷)만 있고 이더리움 네트워크가 없는 이유가 여기 있다. 이 선택은 영구적이지 않다 — `IBlockchainAdapter`가 존재하는 핵심 이유 중 하나다. Phase 2에서 다른 체인이 필요해지면 어댑터 구현체만 교체한다.
