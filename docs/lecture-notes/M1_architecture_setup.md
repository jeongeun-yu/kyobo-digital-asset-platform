...
3. **스켈레톤이 이미 확장 구조를 반영하고 있다**: `ExternalVASPAdapter` (Phase 1, 외부 API 호출)와 `KyoboVASPAdapter` (향후 내재화 시 구현할 stub)는 동일한 `IVASPAdapter` 인터페이스를 구현한다. 커리큘럼에서 이 전체 구조를 구현함으로써, 수강자는 향후 내재화 전환이 코드 어디를 건드리는 것인지 직접 확인하게 된다.
...
![alt text](M1_event_journey_overview.png)
