...
// ────────────────────────────────────────────────────────────────────────
// 실습 1: AuditLogService — SHA-256 체인 방식 감사 로그
//
// 체인 원리:
...
export class AuditLogService {
  ...
  private async _doInsert(params: LogParams): Promise<number> {
    ...
    // 1. 마지막 레코드의 checksum 조회 (체인 연결)
    const lastRow = this.rows[this.rows.length - 1];
    const prevChecksum = lastRow?.checksum ?? '';  // 첫 번째 레코드: 빈 문자열

    // 2. 체인 방식 checksum 계산
    const checksum = this._generateChainedChecksum(
      prevChecksum,
      eventTime,
      params.actor,
      params.action,
  ...
  async verifyIntegrity(id: number): Promise<VerifyResult> {
    ...
    if (idx === -1) throw new Error(`Audit log not found: ${id}`);

    const row = this.rows[idx]!;
    const storedChecksum = row.checksum;

    // 이전 레코드의 checksum 조회
    const prevRow = idx > 0 ? this.rows[idx - 1] : null;
    const prevChecksum = prevRow?.checksum ?? '';
  ...
  async verifyChainIntegrity(): Promise<ChainVerifyResult> {
    ...
    for (const row of this.rows) {
      ...
      if (expected !== row.checksum) {
        return { valid: false, firstInvalidId: row.id, checkedCount: this.rows.length };
      }
  ...
  queryByTimeRange(from: Date, to: Date, options: QueryOptions = {}): AuditLogRow[] {
    const { limit = 100, offset = 0, orderBy = 'desc' } = options;
    const filtered = this.rows.filter(
      r => r.eventTime >= from && r.eventTime <= to,
    );
    const ordered = orderBy === 'asc' ? filtered : [...filtered].reverse();
    return ordered.slice(offset, offset + limit);
  }

  // ── 테스트 전용: 내부 rows 직접 조작 (변조 시뮬레이션) ───────────────
