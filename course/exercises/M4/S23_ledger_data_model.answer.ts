...
export class LedgerService {
  ...
  async recordProcessedEvent(
    ...
    // UNIQUE (txHash, logIndex) — 중복이면 { skipped: true }
  ...
  async addHolding(userId: string, tokenId: bigint, policyId: string): Promise<void> {
    ...
    const exists = this.userNftHoldings.some(
      h => h.userId === userId && h.tokenId === tokenId,
    );
    if (exists) return;  // 중복 → 조용히 무시
  ...
  private async _appendAuditLog(params: {
    ...
    const raw = [
      ...
      params.action,
      params.resourceId,
      safeStringify(params.afterState),
    ].join('');
