...
/**
 * TODO [1]: requireRole 미들웨어 구현
 *
 * 요구사항:
 *   - requiredRole 문자열을 받아 RequestContext를 검사하는 함수를 반환
 *   - ctx.user.roles 배열에 requiredRole이 없으면 에러를 던져야 함
 *   - 에러 메시지: `Unauthorized: ${requiredRole} required`
 *   - 에러 객체의 statusCode 프로퍼티를 401로 설정
 *
 * 힌트:
 *   - 커링(currying) 패턴: (requiredRole) => (ctx) => { ... }
...
export class AdminController {
  ...
  /**
   * TODO [2]: pauseContract 구현
   *
   * 요구사항:
   *   1. requireRole('ADMIN_ROLE')(ctx) 호출로 권한 확인
   *   2. this.contract.pause() 호출 → tx 반환
   *   3. tx.wait() 호출 (컨트랙트 상태 반영)
   *   4. this.auditLog.log() 호출:
   *      - actor: ctx.user.id
  ...
  /**
   * TODO [3]: unpauseContract 구현
   *
   * 요구사항:
   *   1. requireRole('ADMIN_ROLE')(ctx) 권한 확인
   *   2. this.contract.unpause() → tx.wait()
   *   3. 감사 로그: action='CONTRACT_UNPAUSED', afterState: { paused: false }
  ...
  /**
   * TODO [4]: triggerReconcile 구현
   *
   * 요구사항:
   *   1. requireRole('ADMIN_ROLE')(ctx) 권한 확인
   *   2. this.reconcileService.reconcile() 호출
   *   3. 감사 로그: action='RECONCILE_TRIGGERED',
   ...
   * 요구사항:
   *   1. requireRole('ADMIN_ROLE')(ctx) 권한 확인
   *   2. ethers.id(role) 로 bytes32 roleHash 계산
   *   3. this.contract.grantRole(roleHash, address) → tx.wait()
   *   4. 감사 로그: action='ROLE_GRANTED',
   *      resourceId: address, afterState: { role, address }
   *   5. { success: true, txHash: tx.hash } 반환
   */
  async grantRole(
    ctx: RequestContext,
  ...
  /**
   * TODO [7]: revokeRole 구현
   *
   * 요구사항:
   *   1. requireRole('ADMIN_ROLE')(ctx) 권한 확인
   *   2. ethers.id(role) → roleHash
   *   3. this.contract.revokeRole(roleHash, address) → tx.wait()
   *   4. 감사 로그: action='ROLE_REVOKED',
   *      resourceId: address, afterState: { role, address }
   *   5. { success: true, txHash: tx.hash } 반환
   */
  async revokeRole(
