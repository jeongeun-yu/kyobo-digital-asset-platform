...
export class SimulatedGnosisSafe {
   ...
   *
   * EIP-712 domainSeparator = keccak256(abi.encode(DOMAIN_TYPEHASH, chainId, address))
   *
   * 힌트:
  ...
  /**
   * TODO [2]: SafeTx 구조체 해시 계산
   *
   * structHash = keccak256(abi.encode(SAFE_TX_TYPEHASH, to, value, keccak256(data), operation, ...))
   *
   * 힌트:
   *   - bytes 타입(data)은 먼저 ethers.keccak256(params.data) 로 해시
   *   - 나머지 필드 기본값: safeTxGas=0n, baseGas=0n, gasPrice=0n,
   *     gasToken=ZeroAddress, refundReceiver=ZeroAddress, nonce=this._nonce
   *   - 인코딩 타입: ['bytes32','address','uint256','bytes32','uint8',
  ...
  /**
   * TODO [4]: execTransaction 구현
   *
   * 요구사항:
   *   1. getTransactionHash(params)로 safeTxHash 계산
   *   2. 각 서명에 대해 verifySignature로 서명자 주소 복원
   *   3. 복원된 주소 == signer이고 this._owners에 포함된 서명자만 유효
   *   4. validSigners.size < this._threshold → GS020 에러
   *      에러 메시지: `GS020: Insufficient signatures. Got N, required M`
   *   5. nonce 증가 (this._nonce++)
   *   6. { txHash: `0x${randomBytes(32).toString('hex')}`, success: true } 반환
   */
  execTransaction(
    params: SafeTxParams,
