...
/**
 * TODO [1]: withExponentialBackoff 구현
 *
 * 요구사항:
 *   - maxRetries+1 번까지 fn() 시도
 *   - 실패 시 opts.onRetry?.(attempt, err) 호출
 *   - baseDelayMs > 0이면 baseDelayMs * 2^(attempt-1) 대기 (지수 백오프)
 *   - maxRetries 초과 시 마지막 에러 throw
 *   - 성공 시 결과 반환
 *
 * 힌트:
...
export class MockIssuerService {
  ...
  /**
   * TODO [2]: issueSingleWithRetry 구현
   *
   * 요구사항:
   *   1. withExponentialBackoff으로 this.vaspApi.mint() 재시도
   *   2. 성공: this.ledger.updateStatus('SUBMITTED', { vaspTxId: result.txId })
   *           { success: true, txId: result.txId } 반환
   *   3. 실패(모든 재시도 소진): this.ledger.updateStatus('FAILED')
   *                              this.dlq.push(requestId, err.message)
   *                              { success: false } 반환
   *
   * 힌트:
   *   - try/catch 구조
   *   - baseDelayMs: this.baseDelayMs 전달
   *   - onRetry 콜백에서 console.log(`[Retry N] ...`) 출력
   */
