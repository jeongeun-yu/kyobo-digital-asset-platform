...
export class KeyGovernanceService {
  ...
  async proposeTx(proposer: string, params: SafeTxParams): Promise<PendingTx> {
    ...
    await this.auditLog.log({
      actor: proposer, action: 'TX_PROPOSED',
      resourceType: 'PENDING_TX', resourceId: pendingTx.id,
      afterState: pendingTx,
    });

    await this.notifier.send({
      type: 'SIGNATURE_REQUESTED', txId: pendingTx.id, proposer,
      requiredSignatures: threshold, txHash,
    });
  ...
  async addSignature(txId: string, signer: string, signature: string): Promise<SignatureStatus> {
    ...
    const newSigs = [...tx.collectedSignatures, { signer, signature, signedAt: new Date() }];
    ...
    await this.auditLog.log({
      actor: signer, action: 'SIGNATURE_ADDED',
      resourceType: 'PENDING_TX', resourceId: txId,
      afterState: { collected, required, status: newStatus },
    });

    if (ready) {
      await this.notifier.send({ type: 'TX_READY_TO_EXECUTE', txId, collected, required });
    }
  ...
  async executeTx(txId: string, executor: string): Promise<{ onChainTxHash: string }> {
    ...
    if (tx.status !== 'READY_TO_EXECUTE') {
      throw new Error(
        `TX not ready to execute. Status: ${tx.status}. ` +
        `Collected: ${tx.collectedSignatures.length}, Required: ${tx.requiredSignatures}`,
      );
    }
    ...
    await this.auditLog.log({
      actor: executor, action: 'TX_EXECUTED',
      resourceType: 'PENDING_TX', resourceId: txId,
  ...
  async cancelTx(txId: string, cancelledBy: string, reason: string): Promise<void> {
    const tx = await this.db.findById(txId);
    if (!tx) throw new PendingTxNotFoundError(txId);
    if (tx.status === 'EXECUTED') throw new Error('Cannot cancel already-executed transaction');

    await this.db.updateStatus(txId, 'CANCELLED');

    await this.auditLog.log({
      actor: cancelledBy, action: 'TX_CANCELLED',
      resourceType: 'PENDING_TX', resourceId: txId,
