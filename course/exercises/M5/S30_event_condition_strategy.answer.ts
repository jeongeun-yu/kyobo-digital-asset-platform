...
// ────────────────────────────────────────────────────────────────────────
// EventConditionService — 전략 레지스트리
//
// strategies: Map<eventType, IConditionStrategy>
//   → 새 이벤트 추가 시 이 파일은 건드리지 않는다 (OCP)
//   → 같은 eventType 재등록 시 나중 전략으로 덮어쓰기
// ────────────────────────────────────────────────────────────────────────
...
//
// 지원: WALK_GOAL_MET, HEALTH_CHECK_DONE
//
// tokenId 비트 레이아웃 (M6 KyoboNFT.sol과 반드시 동기화):
//   tokenId = (productCode << 64) | eventCode
//   WALK:   productCode = 0x01
//   HEALTH: productCode = 0x02
// ────────────────────────────────────────────────────────────────────────
...
export class ActivityConditionStrategy implements IConditionStrategy {
  ...
  async evaluate(event: ActivityEvent): Promise<ConditionResult> {
    ...
    // HEALTH_CHECK_DONE: 완료 여부만 체크 — steps 불필요, 항상 통과
...
//
// 지원: COUPON_CLAIM, CAMPAIGN_REWARD
// 외부 eligibilityChecker에 자격 조회 위임 (Stub으로 교체 가능)
// ────────────────────────────────────────────────────────────────────────
...
export class CouponConditionStrategy implements IConditionStrategy {
  ...
  async evaluate(event: ActivityEvent): Promise<ConditionResult> {
    ...
    const tokenId =
      (CouponConditionStrategy.PRODUCT_COUPON << CouponConditionStrategy.PRODUCT_CODE_SHIFT)
      | BigInt(event.eventCode);
...
//
// 지원: PREMIUM_PAID
// event.data.amount >= threshold 이면 발행 허가
// ────────────────────────────────────────────────────────────────────────
...
(async () => {
  ...
  // tokenId 비트 인코딩 검증 (M6 KyoboNFT.sol과 동기화 확인)
  ...
  // ── [6] OCP 검증 — 새 전략 추가 시 EventConditionService 코드 변경 없음 ──
