...
export class ActivityConditionStrategy implements IConditionStrategy {
  ...
  async evaluate(event: ActivityEvent): Promise<ConditionResult> {
    ...
    const tokenId = (productCode << ActivityConditionStrategy.PRODUCT_CODE_SHIFT) | BigInt(event.eventCode);
...
(async () => {
  ...
  // HEALTH_CHECK_DONE은 data 없어도 통과
  ...
  // Mock: 호출 사실 자체를 검증 (isEligible이 실제 호출됐는가)
  let callCount = 0;
  const mockChecker = {
