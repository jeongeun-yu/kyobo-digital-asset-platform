...
// ────────────────────────────────────────────────────────────────────────
// Slither 심각도 분류 정의 (타입/상수 — 수정하지 말 것)
// ────────────────────────────────────────────────────────────────────────
...
// ────────────────────────────────────────────────────────────────────────
// TODO 1: simulateVulnerableBurn 구현
//
// KyoboNFT.burn()의 취약한 실행 순서 시뮬레이션 (Interactions → Effects 순서).
//
// 구현 지시:
//   1. balances.get(attacker)로 현재 잔액을 확인한다.
//   2. 잔액이 0 이하면 'success'를 즉시 반환한다.
//   3. [❌ 취약한 순서] 외부 호출(Interactions) 먼저:
//      callCount.value < maxReentrancy 이면 callCount.value++하고 재귀 호출한다.
//      (이 시점에서 잔액이 아직 차감되지 않아 재진입이 추가 인출을 허용함)
//   4. [❌ 너무 늦은 Effects] 재귀 호출 후 잔액을 1 차감한다.
//   5. callCount.value >= maxReentrancy 이면 'reentrancy_exploited', 아니면 'success' 반환.
// ────────────────────────────────────────────────────────────────────────
...
// ────────────────────────────────────────────────────────────────────────
// TODO 2: simulateSafeBurn 구현
//
// CEI 패턴을 올바르게 적용한 안전한 burn 시뮬레이션.
//
// 구현 지시:
//   1. 잔액 확인 (Check): balances.get(attacker)로 현재 잔액을 가져온다.
//   2. 잔액이 0 이하면 { success: false, burnedCount: 0 } 반환.
//   3. [✅ Effects 먼저] 잔액을 즉시 1 차감한다 (재진입 시 잔액 이미 감소).
//   4. [✅ Interactions 나중에] callCount.value < maxReentrancy 이면
//      callCount.value++하고 재귀 호출한다.
//      (재진입 시점에 잔액이 이미 차감됐으므로 추가 인출 불가)
//   5. { success: true, burnedCount: 1 } 반환.
// ────────────────────────────────────────────────────────────────────────
...
// ────────────────────────────────────────────────────────────────────────
// TODO 3: checkWithTxOrigin 구현
//
// tx.origin 기반 권한 체크 — 취약한 패턴.
//
// 구현 지시:
//   - txOrigin === owner 비교 결과를 반환한다.
//   - msgSender는 사용하지 않는다 (취약점의 핵심).
// ────────────────────────────────────────────────────────────────────────
...
// ────────────────────────────────────────────────────────────────────────
// TODO 4: checkWithMsgSender 구현
//
// msg.sender 기반 권한 체크 — 안전한 패턴.
//
// 구현 지시:
//   - msgSender === owner 비교 결과를 반환한다.
//   - txOrigin은 사용하지 않는다.
// ────────────────────────────────────────────────────────────────────────
...
// ────────────────────────────────────────────────────────────────────────
// TODO 5: classifyFindings 구현
//
// SlitherFinding 배열을 심각도별로 분류하여 반환한다.
//
// 구현 지시:
//   1. 결과 객체 result를 초기화한다: { HIGH: [], MEDIUM: [], LOW: [], INFORMATIONAL: [] }
//   2. findings 배열을 순회하며 각 finding을 result[finding.severity]에 push한다.
//   3. result를 반환한다.
// ────────────────────────────────────────────────────────────────────────
