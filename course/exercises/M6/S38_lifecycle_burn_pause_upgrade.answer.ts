...
// KyoboNFT 생명주기 시뮬레이터
//
// burn의 Pause 예외 처리 정책:
//   정책 A (현재 KyoboNFT.sol): _update에 whenNotPaused → burn도 차단
//   정책 B (운영 팀 선택 가능): burn만 Pause 예외 처리 → 피해 NFT 즉시 회수 가능
// ────────────────────────────────────────────────────────────────────────
...
export class KyoboNFTLifecycle {
  ...
  /**
   * _update hook 구현 — ERC-1155 내부 함수
   *
   * KyoboNFT.sol 현재 구현 (정책 A):
   *   function _update(...) internal override whenNotPaused { ... }
   *
   * 정책 B (burn 예외 처리):
   *   if (!isBurn) { _requireNotPaused(); }
   *
   * from === '' (address(0) 대응): mint
   * to   === '' (address(0) 대응): burn
   * otherwise: transfer
   */
  private _update(from: string, to: string): void {
    const isMint     = from === '';   // address(0) = mint
    const isBurn     = to === '';     // address(0) = burn
    ...
    if (this.burnPauseExempt) {
      ...
      // 정책 A (기본): mint/burn/transfer 모두 Pause 차단
      if (this.state === 'PAUSED') {
        throw new Error('EnforcedPause: contract is paused');
      }
    }
    void isMint; // 사용하지 않는 변수 경고 제거
  }
  ...
  /**
   * burn — MINTER_ROLE 전용
   * whenNotPaused 없음 (함수 레벨) — _update hook에서 정책별 제어
   *
   * KyoboNFT.sol:
   ...
   *   }
   */
  burn(caller: string, from: string, tokenId: bigint, amount: bigint): void {
    this.requireRole('MINTER_ROLE', caller);
    this._update(from, '');  // to='' → burn
    ...
    if (balance.amount < amount) throw new Error('KyoboNFT: insufficient balance for burn');
  ...
  /**
   * _authorizeUpgrade — UPGRADER_ROLE 전용
   * KyoboNFT.sol:
   *   function _authorizeUpgrade(address) internal override onlyRole(UPGRADER_ROLE) {}
   */
  authorizeUpgrade(caller: string, _newImpl: string): void {
    this.requireRole('UPGRADER_ROLE', caller);
    this.state = 'UPGRADED';
  }
