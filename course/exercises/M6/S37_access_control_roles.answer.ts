...
export class KyoboNFTSimulator {
  ...
  constructor(adminAddress: string) {
    // initialize()에서 admin에게 모든 역할 부여
    this._grantRole('DEFAULT_ADMIN_ROLE', adminAddress);
    this._grantRole('MINTER_ROLE',        adminAddress);
    this._grantRole('PAUSER_ROLE',        adminAddress);
    this._grantRole('UPGRADER_ROLE',      adminAddress);
  }
  ...
  private _requireNotPaused(): void {
    if (this.paused) throw new Error('EnforcedPause: contract is paused');
  }

  // ── 발행 ────────────────────────────────────────────────────────────

  /**
   * mint — onlyRole(MINTER_ROLE) whenNotPaused
   * KyoboNFT.sol:
  ...
  mint(caller: string, to: string, tokenId: bigint, amount: bigint): void {
    ...
    if (amount <= BigInt(0)) throw new Error('KyoboNFT: zero amount');
    this.tokens.push({ owner: to, tokenId, amount });
  }

  /**
   * mintBatch — onlyRole(MINTER_ROLE) whenNotPaused
   * KyoboNFT.sol:
   *   function mintBatch(address[] calldata to, uint256[] calldata tokenIds, uint256[] calldata amounts)
   *     external onlyRole(MINTER_ROLE) whenNotPaused
   *
   * 주의: ERC-1155 표준 mintBatch(1명에게 여러 종류)와 다름
   *       KyoboNFT: 여러 명 각각에게 발행
   */
...
(async () => {
  ...
  const GAS_PER_MINT = 50_000;  // ERC-1155 단건 발행 가스 (강의 노트 기준)
  const BLOCK_GAS_LIMIT = 30_000_000;
  const RECOMMENDED_MAX = 25_000_000;  // 블록 한도의 83%
  ...
  check(`mintBatch ${BATCH_SIZE}건 실행 성공`, nft.totalMinted() === BATCH_SIZE + 1);  // +1 from earlier mint
  ...
  // MINTER는 PAUSER_ROLE 없음 → pause 불가
  expectThrows(
  ...
  // ADMIN이 MINTER_ROLE 보유자 → mint 가능
  ...
  // ── [7] Nonce 순차 처리 원칙 (개념 검증) ─────────────────────────────
  console.log('\n[검증 7] TX Nonce 순차 처리 — RBF 패턴');

  interface TxRecord { nonce: number; gasPrice: bigint; status: 'pending' | 'confirmed' | 'replaced'; chunk: number }

  const txQueue: TxRecord[] = [
    { nonce: 10, gasPrice: BigInt(20e9), status: 'pending', chunk: 0 },
    { nonce: 11, gasPrice: BigInt(20e9), status: 'pending', chunk: 1 },
    { nonce: 12, gasPrice: BigInt(20e9), status: 'pending', chunk: 2 },
  ];

  // TX #0이 Stuck → nonce 10이 처리 안 됨 → TX #1, #2 대기
  const stuckTx = txQueue[0]!;
  const canProcess1 = txQueue[1]!.nonce === stuckTx.nonce + 1;  // nonce 11은 10 처리 후 가능
  check('nonce 11 TX는 nonce 10 처리 전까지 대기 (순차 처리 원칙)', canProcess1);

  // RBF: 가스비 1.2배로 대체
  const rbfGasPrice = (stuckTx.gasPrice * BigInt(12)) / BigInt(10);
  stuckTx.status = 'replaced';
  const rbfTx: TxRecord = { nonce: 10, gasPrice: rbfGasPrice, status: 'confirmed', chunk: 0 };
  check(
    `RBF 적용: 가스비 ${stuckTx.gasPrice}→${rbfGasPrice} (1.2배), nonce 10 교체 완료`,
    rbfGasPrice === BigInt(24e9),
  );
  check('RBF 후 nonce 10 confirmed → 후속 TX 진행 가능', rbfTx.status === 'confirmed');
