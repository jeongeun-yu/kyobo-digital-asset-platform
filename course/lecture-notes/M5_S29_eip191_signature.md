...
>
> **Phase 1(월렛원 수탁)에서는 해당 없음.** 수탁 지갑은 VASP가 직접 생성하므로 별도 소유권 증명이 불필요합니다.  
...
S28에서 지갑 등록 방식이 수탁/비수탁으로 나뉜다고 배웠다. 수탁(월렛원)은 VASP가 주소를 직접 제공하므로 소유권이 자명하다. 문제는 **비수탁** 방식이다 — 사용자가 자신의 지갑 주소를 직접 제출할 때다.
...
수탁(월렛원) 방식에서는 이 공격이 불가능하다 — VASP가 지갑을 직접 생성하므로 사용자가 임의 주소를 제출할 수 없다. 비수탁 방식에서만 이 문제가 발생한다.
