...
> **[비수탁 지갑 전용]** 이 세션은 사용자가 MetaMask 등 자가관리 지갑을 교보 시스템에 등록하는 시나리오를 다룹니다.
>
> **Phase 1(VASP 수탁)에서는 해당 없음.** 수탁 지갑은 VASP가 직접 생성하므로 별도 소유권 증명이 불필요합니다.  
> 비수탁 지갑 지원을 추가하는 시점에 이 세션의 내용이 적용됩니다.

> 모듈 5 · 세션 29 · 1시간  
> 구현 위치: `internal/apps/issuer-service/src/services/WalletMappingService.ts` → `verifyOwnership()`

---

## 강의 파트 (20분)

### 1. 비수탁 지갑 등록에서 "내 지갑 주소입니다"라는 말만으로는 부족하다

S28에서 지갑 등록 방식이 수탁/비수탁으로 나뉜다고 배웠다. 수탁(VASP)은 VASP가 주소를 직접 제공하므로 소유권이 자명하다. 문제는 **비수탁** 방식이다 — 사용자가 자신의 지갑 주소를 직접 제출할 때다.

**Sybil 공격 시나리오:**

비수탁 지갑 등록 흐름:
```
사용자: "내 지갑 주소는 0xABCD...입니다" → 서버에 제출
서버: 그대로 저장?
```

주소만 받아서 저장하면 아래 공격이 가능하다.
...
수탁(VASP) 방식에서는 이 공격이 불가능하다 — VASP가 지갑을 직접 생성하므로 사용자가 임의 주소를 제출할 수 없다. 비수탁 방식에서만 이 문제가 발생한다.
