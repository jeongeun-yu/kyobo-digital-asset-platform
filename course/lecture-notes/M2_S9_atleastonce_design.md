...
> Block B — 이벤트 파이프라인 · M2 S9 · 강의 55분  
...
> **비유 한계 주의**: "손님에게 확인"은 비유의 단순화다. 실제 시스템은 사람이 개입하지 않는다. `processed_events` 테이블의 `event_id UNIQUE` 제약 조건이 중복 처리를 **자동으로** 차단한다. 동일 이벤트가 재수신되면 INSERT 시 `ON CONFLICT DO NOTHING`으로 즉시 걸러지고, XACK만 전송하고 종료한다.
```
