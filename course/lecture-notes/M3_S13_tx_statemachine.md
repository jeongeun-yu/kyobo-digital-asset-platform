...
- [ ] Observer Pattern — `svc.on('transition', handler)` 구독 후 `submitMintRequest()` 시 이벤트 수신 확인
- [ ] `TxTransitionEvent` 구조 — `{ requestId, from, to, req }` 필드 모두 포함 확인
- [ ] Observer 패턴 장점 — 서비스 코드 무변경으로 감사 로그·알림·메트릭 구독자 추가 설명

---

---

## Observer Pattern — TxTransitionEvent 구독

`TxStateMachineService`는 `EventEmitter`를 extends한다. 상태 전이가 발생할 때마다 `'transition'` 이벤트를 emit한다.

```typescript
// TxStateMachineService.ts
export interface TxTransitionEvent {
  requestId: string;
  from:      TxStatus;
  to:        TxStatus;
  req:       MintRequest;  // 전이 시점의 MintRequest 스냅샷
}

export class TxStateMachineService extends EventEmitter {
  private async _transition(req: MintRequest, to: TxStatus, extra?: Partial<MintRequest>): Promise<void> {
    const from = req.status;
    await this.repo.updateStatus(req.id, to, extra);
    this.emit('transition', { requestId: req.id, from, to, req: { ...req, status: to } });
  }
  // 모든 상태 전이는 _transition()을 통과 → emit 보장
}
```
...
```typescript
const svc = new TxStateMachineService(repo, vaspClient, walletResolver);
...
// 단일 이벤트만 수신
svc.once('transition', (evt: TxTransitionEvent) => {
  // 첫 번째 전이만 처리
});

// 구독 해제
const handler = (evt: TxTransitionEvent) => { ... };
svc.on('transition', handler);
svc.off('transition', handler);
```
...
❌ 직접 호출 방식:
  TxStateMachineService.handleMined() {
    ...
    await auditLog.record(...)     // ← 강결합
    await dashboard.notify(...)    // ← 강결합
    await alertService.send(...)   // ← 강결합
  }
  → 새 구독자 추가 = 핵심 서비스 코드 수정 필요

✅ Observer 방식:
  TxStateMachineService.handleMined() {
    await this._transition(req, 'MINED')  // emit만 함
  }

  // 구독자는 서비스 외부에서 독립적으로 등록
  svc.on('transition', auditLog.record)
  svc.on('transition', dashboard.notify)
  svc.on('transition', alertService.send)
  → TxStateMachineService 코드 무변경
  → 구독자 추가/제거 자유
```
...
세 구독자 모두 `TxStateMachineService` 코드를 단 한 줄도 수정하지 않는다.
