...
│ HMAC-SHA256         │ 대칭키(공유 secret)               │ ✅ 업계 표준         │
│ RSA/ECDSA           │ 비대칭키 (공개키/개인키)          │ 가능, 오버킬 경우多  │
│ IP 화이트리스트     │ 발신 IP 대역 제한                 │ 보완 수단 (단독 불가)│
│ API Key (Bearer)    │ 헤더에 토큰 포함                  │ 단순 인증에 사용     │
│ mTLS                │ 클라이언트 인증서 상호 검증       │ ✅ 금융권 고신뢰 구간│
│ VASP 자체 서명      │ VASP별 독자 방식                  │ VASP 계약 시 협의    │
└─────────────────────┴──────────────────────────────────┴──────────────────────┘
```
...
→ 업계 표준. 검증 라이브러리와 레퍼런스가 풍부하고 구현이 단순.

**VASP가 HMAC을 지원하지 않는 경우**

Phase 1 파트너(월렛원)의 실제 인증 방식은 계약 시 확정된다.  
VASP마다 지원 방식이 다르고, 일부는 HMAC 대신 아래를 사용한다:

```
월렛원 (Phase 1)   → 계약 협의 필요 (HMAC or 자체 토큰)
Circle (Phase 2)   → Circle 자체 서명 방식
구형 VASP          → IP 화이트리스트만 지원하는 경우도 있음
고보안 구간        → mTLS (클라이언트 인증서 상호 검증)
```

**이 과정에서 HMAC을 배우는 핵심 이유는 방식 자체가 아니라 구조다.**

```
어떤 인증 방식을 쓰든 코드 변경 위치는 한 곳:
  _verifySignature() 내부 → 또는 IVASPAdapter 구현체

WebhookServer, RedisStreamPublisher, ConsumerGroupWorker는
인증 방식이 바뀌어도 한 줄도 수정하지 않는다.
```

M8에서 EIP-712 SafeTx (비대칭 서명)를 다룰 때 이 차이가 명확하게 드러난다.
