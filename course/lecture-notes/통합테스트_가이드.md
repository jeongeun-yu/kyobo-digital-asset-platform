...
Docker 컨테이너를 실행하기 위한 환경이다. Docker Desktop 대신 **Rancher Desktop**을 권장한다 (라이선스 무료).

1. https://rancherdesktop.io 에서 설치 파일 다운로드
2. 설치 후 실행 → 우측 하단 트레이 아이콘이 초록색이 될 때까지 대기
3. 설치 시 **"Container Engine"** 선택 화면에서 **`dockerd (moby)`** 선택 (기본값 `containerd` 아님)

```powershell
docker --version   # Docker version 26.x.x 이상이면 정상
...
```
# Hardhat 로컬 테스트는 기본값 그대로 사용 가능 — 별도 설정 불필요
...
> `DEPLOYER_PRIVATE_KEY` 미설정 시 `OPERATOR_PRIVATE_KEY`와 동일 키를 사용한다. 단, 통합 테스트에서는 두 키가 다른 역할(setMode / issueActivityNFT)을 동시에 수행하므로 가능하면 분리를 권장한다. 자세한 이유는 [2-6절](#2-6-설계-포인트-signer-분리-nonce-충돌-방지) 참고.
...
## Part 1 — Hardhat 로컬 테스트 (컨트랙트 단위)

Hardhat은 로컬 PC에서 실행되는 가상 블록체인이다.  
...
| **REVERT** | `setMode(1)` | TX 실패. VASP가 에러 반환 → 1차 FAILED → Redis 재시도 → CONFIRMED (one-shot revert) |
...
> **REVERT one-shot 동작**: `MockVASP.setMode(REVERT)`는 한 번 revert 후 자동으로 NORMAL로 복귀한다. Redis Stream consumer가 실패한 메시지를 재시도하면 두 번째 시도에서 정상 처리된다. DB에서 FAILED row와 CONFIRMED row가 함께 보이는 것이 정상이다.
...
**Consumer ID 로그**  
메시지를 처리하는 consumer의 ID가 서버 로그에 함께 출력된다. 어느 consumer instance가 어떤 메시지를 처리했는지 추적할 수 있다.
```
...
테스트 실행 전 두 이미지를 빌드해야 한다. **최초 1회만 빌드하면 되며, 소스 변경 시에만 재빌드가 필요하다.**
...
# Java internal-ledger 이미지 (Maven 빌드 포함 — 3~5분 소요)
npm run build:java
```
...
> `build:java`는 Docker 컨테이너 내부에서 Maven 빌드를 실행하므로 Java 17이 로컬에 없어도 된다.
...
**문제**: `vasp.setMode('NO_EMIT')` (DEPLOYER_KEY, nonce 3) 직후 IssuerService가 `issueActivityNFT` 호출(OPERATOR_KEY)을 시도할 때, 두 키가 같으면 동일 nonce를 eth_getTransactionCount에서 읽어온다.
...
> Hardhat 전용 RPC(`evm_setAutomine`, `evm_snapshot`)는 공개 노드에서 사용 불가.
...
> `DEPLOYER_PRIVATE_KEY`와 `OPERATOR_PRIVATE_KEY`를 같은 키로 쓰면 `setMode` TX와 `issueActivityNFT` TX가 동일 nonce를 경쟁할 수 있다. 테스트 시나리오에 따라 nonce 충돌이 발생할 수 있으므로 분리를 권장한다. ([2-6절](#2-6-설계-포인트-signer-분리-nonce-충돌-방지) 참고)
...
> 실행 전 Rancher Desktop이 실행 중이어야 한다. (PostgreSQL·Java·Redis 컨테이너 기동)  
...
**Consumer ID 로그**  
각 메시지 처리 시작 시 어느 consumer가 담당하는지 로그로 확인할 수 있다:
```json
{"consumer":"activity-1","group":"activity-consumers","messageId":"1779920914446-0","eventType":"ACTIVITY_ACHIEVED","attempt":1}
```
...
| 항목 | Hardhat 스크립트 | mock-vasp 통합 테스트 | Sepolia 스크립트 | Sepolia 통합 테스트 | Redis Stream 통합 테스트 |
|---|---|---|---|---|---|
