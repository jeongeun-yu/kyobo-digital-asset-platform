...
> 대상: `internal/packages/event-engine/src/dmz/DLQHandler.ts`
...
└── internal/packages/event-engine/src/dmz/DLQHandler.ts
```
...
### BrokenProcessor (항상 실패하는 테스트용 프로세서)

```typescript
// 항상 실패하는 Processor — DLQ 이동 트리거용
class BrokenProcessor implements EventProcessor {
  readonly eventTypes = ['NFT_ISSUED'];
  private callCount = 0;

  async process(_message: StreamMessage): Promise<void> {
    this.callCount++;
    throw new Error(`[BrokenProcessor] 고의 실패 (${this.callCount}번째 시도)`);
  }
}
```
...
```
[메시지 적재]
  XADD kyobo:events * eventType NFT_ISSUED ... requestId req-dlq-001

[워커 실행]
  _handleWithRetry → BrokenProcessor.process() → throw
  → _retryCount: 0 → 1

  _reclaimPending → XAUTOCLAIM → 재수신
  → BrokenProcessor.process() → throw
  → _retryCount: 1 → 2
  ...
  재수신 → _handleWithRetry:
  → retryCount(3) >= MAX_RETRIES(3) 조건 진입
  → dlqHandler.move({ messageId, reason: 'max retries exceeded' })
  → XADD kyobo:events:dlq * ...원본필드... _reason=max retries exceeded
  → XACK kyobo:events (원 스트림 PEL 정리)

[DLQ 확인]
  listPending() → [{ messageId, reason: 'max retries exceeded', ... }]

[원인 파악 후 재큐잉]
  requeueMessage(dlqMessageId)
  → XADD kyobo:events * ...원본필드... _requeuedFrom=<dlqMessageId>
  → XDEL kyobo:events:dlq <dlqMessageId>

[재처리]
  워커가 재큐잉된 메시지 수신 → 이번엔 정상 처리 가능한 Processor로 교체
```
...
async function runDLQScenario() {
  ...
  // 항상 실패하는 Processor
  const brokenProcessor = new BrokenProcessor();
  ...
  const worker = new ConsumerGroupWorker({
    ...
    config: {
      ...
      consumerId: 'worker-s11',
      batchSize:  1,
      blockMs:    10,
      minIdleMs:  0,  // 즉시 재수신 (실습용 — 실제는 30_000)
    },
  ...
  // 메시지 1개 적재
  await mockRedis.xadd('kyobo:events', {
  ...
  // 워커 실행 (3회 실패 → DLQ 이동)
  worker.start();
  await sleep(500);  // 3회 처리 시간 대기
  worker.stop();

  // Part 1 검증: DLQ에 1건
  const dlqItems = await dlqHandler.listPending(10);
  console.assert(dlqItems.length === 1, `expected 1 DLQ item, got ${dlqItems.length}`);
  console.log(`[Part 1] DLQ 항목 수: ${dlqItems.length} ✅`);
  console.log(`  reason: ${dlqItems[0].reason}`);
  console.log(`  event:  ${dlqItems[0].event['eventType']}`);

  // Part 2: requeueMessage
  const dlqEntry = dlqItems[0];
  // DLQ에서 원본 messageId는 _originalMessageId 필드에 있음
  const dlqMessageId = dlqEntry.event['_originalMessageId'] ?? dlqEntry.messageId;

  const { newMessageId } = await dlqHandler.requeueMessage(dlqMessageId);
  console.log(`[Part 2] 재큐잉 완료: newMessageId=${newMessageId} ✅`);

  // 재큐잉 후 DLQ 확인
  const dlqAfter = await dlqHandler.listPending(10);
  console.assert(dlqAfter.length === 0, `expected 0 DLQ items after requeue, got ${dlqAfter.length}`);
  console.log(`[Part 2] DLQ 항목 수 (재큐잉 후): ${dlqAfter.length} ✅`);
}
```
...
- [ ] Part 1: dlqHandler.listPending() → 항목 1개 확인
- [ ] Part 1: DLQ 항목의 `reason`이 `max retries exceeded` 포함 확인
- [ ] Part 2: `[requeue] 재큐잉 완료` + DLQ 항목 수 0 확인
- [ ] 알림 서비스 장애 시에도 DLQ 이동이 성공하는지 확인 (`.catch()` 패턴 이해)
- [ ] xadd + xdel이 원자적이지 않은 이유와 멱등성으로 보완하는 방법 설명 가능
