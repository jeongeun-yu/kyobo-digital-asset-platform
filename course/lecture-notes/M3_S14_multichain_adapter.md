...
## 11. Factory Pattern — 환경변수 기반 어댑터 생성

Strategy Pattern이 "어떻게 쓰는가"를 정의한다면, Factory Pattern은 "어떻게 만드는가"를 담당한다.

```
Strategy:  IssuerService → IBlockchainAdapter 인터페이스만 알면 됨
Factory:   ChainAdapterFactory → 환경변수/설정에서 올바른 구현체를 생성
```

#### ChainAdapterFactory 구조

```typescript
// chain-adapters/src/ChainAdapterFactory.ts
export class ChainAdapterFactory {
  static create(config: AdapterConfig): IBlockchainAdapter {
    switch (config.chainType) {
      case 'EVM':  return new EVMAdapter({ rpcUrl: config.rpcUrl, ... });
      case 'XRPL': return new XRPLAdapter({ wsUrl: config.wsUrl, ... });
      case 'BFT':  return new CircleAdapter({ apiKey: config.apiKey, ... });
      case 'UTXO': return new UTXOAdapter({ rpcUrl: config.rpcUrl, ... });
      default:
        const _exhaustive: never = config;  // TypeScript exhaustive check
        throw new UnsupportedChainError(config.chainType);
    }
  }

  static createFromEnv(chainType: ChainType): IBlockchainAdapter {
    return ChainAdapterFactory.create({ chainType, ...envValues });
  }
}
```
...
```typescript
// 새 체인 타입 'COSMOS' 추가 시 case 처리 없으면 컴파일 에러
// → 런타임 버그 대신 컴파일 타임에 누락 감지
const _exhaustive: never = config;  // ERROR: 'COSMOS'를 'never'에 할당 불가
```
...
```
호출 코드 (IssuerService):
  this.adapter.mintNFT(params)    ← Strategy (무엇을 쓸지는 모름)

설정/초기화 코드:
  const adapter = ChainAdapterFactory.createFromEnv('EVM')   ← Factory (어떻게 만들지 결정)
  const issuer  = new IssuerService(adapter)                  ← DI로 주입
```

Factory는 설정 코드와 비즈니스 코드의 경계에 위치한다. Strategy가 비즈니스 코드에서 체인 종류를 모르게 만들고, Factory가 설정 코드에서 구체 클래스 이름을 한 곳에 모은다.

```
❌ Before (설정 코드가 비즈니스 로직에 산재):
  services/IssuerService.ts:   const adapter = new EVMAdapter(...)
  services/LedgerService.ts:   const adapter = new EVMAdapter(...)
  handlers/EventHandler.ts:    const adapter = new EVMAdapter(...)
  → 체인 교체 시 N개 파일 수정

✅ After (Factory 한 곳에서):
  const adapter = ChainAdapterFactory.createFromEnv('EVM')
  → 체인 교체 = Factory의 case 한 줄
```

#### 실습에서 확인할 것

```typescript
// S14 실습 [6] Factory Pattern 섹션
const adapter = ChainAdapterFactory.create({ chainType: 'EVM', rpcUrl: '...', chainId: '11155111' });
adapter.chainType === 'EVM'  // ✅

ChainAdapterFactory.create({ chainType: 'UNKNOWN' as any });
// → UnsupportedChainError 발생 ✅
```
