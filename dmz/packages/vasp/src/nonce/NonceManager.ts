 ...
 *
 * Custody Track Session 3/5 참조: Nonce 갭 감지·복구, Nonce 전략 분류
 */
...
/**
 * TODO (Phase 3): Nonce 할당, 추적, 갭 감지·복구 전담
 *
 * 핵심 제약:
...
export class NonceManager {
  ...
  async allocate(
    ...
    throw new Error('TODO: Phase 3 — NonceManager.allocate 구현 필요');
  }

  /**
   * TODO (Phase 3): Nonce 해제 (TX DROPPED/FAILED 시)
   *   해제된 Nonce는 다시 할당 가능 또는 빈 TX로 채움
   */
  async release(_address: string, _chainId: string, _nonce: number): Promise<void> {
    throw new Error('TODO: Phase 3 — NonceManager.release 구현 필요');
  }

  /**
   * TODO (Phase 3): Nonce 갭 감지
  ...
  async detectGap(
    ...
    throw new Error('TODO: Phase 3 — NonceManager.detectGap 구현 필요');
  }

  /**
   * TODO (Phase 3): gas bump — 동일 Nonce로 gasPrice 1.2배 재전송 (EIP-1559: maxFee 증가)
  ...
  async bumpGas(
    ...
    throw new Error('TODO: Phase 3 — NonceManager.bumpGas 구현 필요');
  }
}
