...
export class WalletMappingService {
  ...
  async getWalletAddr(userId: string): Promise<string> {
    ...
    // TODO (M5 S25 실습): VASP 타입 분기 + 프로비저닝
    //   const vaspType = this._resolveVaspType(userId);
    //   if (vaspType === 'EXTERNAL') {
    //     const { walletAddr } = await this.externalVasp.provisionWallet(userId);
    //     await this.repo.save({ userId, walletAddr, vaspType: 'EXTERNAL', verified: false, createdAt: new Date() });
    //     return walletAddr;
    //   }
  ...
  async verifyOwnership(params: {
    ...
    //   await this.repo.save({ userId, walletAddr, vaspType: 'EXTERNAL', verified: true, createdAt: new Date() });
    //   return true;
